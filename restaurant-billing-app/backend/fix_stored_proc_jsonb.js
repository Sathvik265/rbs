...
async function updateFunction() {
    ...
    try {
        console.log("Updating function move_orders_to_bill_json for JSONB...");

        // Logic: 
        // i.category is now JSONB (e.g. {"name": "Dosa", "qty": 1}).
        // We want to return a list of categories in the bill JSON.
        // So if i.category is present, we wrap it in an array [ {name...} ].
        // If null, we return empty array [].
        // So: COALESCE(jsonb_build_array(i.category), '[]'::jsonb) -- NO, if i.category is null build_array might include null?
        // Better: CASE WHEN i.category IS NOT NULL THEN jsonb_build_array(i.category) ELSE '[]'::jsonb END.

        // Note: If i.category was ALREADY an array (from some legacy data? unlikely), we might double wrap.
        // But we standardized it to object. So jsonb_build_array is correct to make it a list of 1 category.
...
`;
        ...
        console.log("Successfully updated move_orders_to_bill_json (JSONB version)");
