...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    if (shiftResult.rows.length === 0) {
      return res
        .status(400)
        .json({ detail: "No open shift found for the given track and date." });
    }
    const shift_id = shiftResult.rows[0].shift_id;

    // Create a session with shift_id
    const sessionResult = await pool.query(
      "INSERT INTO sessions (clerk_initials, shift_code, session_date, shift_id) VALUES ($1, $2, $3, $4) RETURNING session_id",
      [initials, track, date, shift_id]
    );
