-- Drop both potential signatures to remove ambiguity
DROP FUNCTION IF EXISTS public.move_orders_to_bill_json(integer, text, text);
DROP FUNCTION IF EXISTS public.move_orders_to_bill_json(integer, character varying, character varying);
...
BEGIN
    ...
    -- We cast table_no and party_no to text to ensure matching
    SELECT jsonb_agg(
        jsonb_build_object(
    ...
    SET items_json = v_bill_items,
        items = v_bill_items -- Update legacy column too just in case
    WHERE id = p_bill_id;
